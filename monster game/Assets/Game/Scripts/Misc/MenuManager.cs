...
namespace RGSK
{
	public class MenuManager : MonoBehaviour
	{
		...
		public GameObject menupanelpage;
		...
		void Start ()
		{
            ...
            Debug.LogError("Shw cash :: " + playerCurrency.text);
		}
		...
		void UpdateUI ()
		{
			...
			if (playerCurrency)
                playerCurrency.text = PlayerData.currency.ToString("N0") + " Cr";
            //playerCurrency.text = ""+ " Cr";
			...
			switch (state) {
				...
				case State.VehicleSelect:
					...
					if (itemPrice)
						itemPrice.text = menuVehicles [vehicleIndex].unlocked ? string.Empty : menuVehicles [vehicleIndex].price.ToString ("N0") + " Cr";
				...
				case State.TrackSelect:
					...
					if (itemPrice)
						itemPrice.text = menuTracks [trackIndex].unlocked ? string.Empty : menuTracks [trackIndex].price.ToString ("N0") + " Cr";
				...
				case State.Customize:
					...
					if (itemPrice)
						itemPrice.text = incartCr.ToString ("N0") + " Cr";
		...
		IEnumerator LoadScene ()
		{
            ...
            Debug.Log("scenename " +menuTracks[trackIndex].sceneName);
			while (!async.isDone) {
				if (loadingBar)
        ...
        public void SHOWiNAPPUI() {
            iNAPPuI.SetActive(true);
           
        }

        public void HideInappUI() {
            iNAPPuI.SetActive(false);
            
        }
        public void Deactivateads() {
          //  FindObjectOfType<Purchaser>().BuyNonConsumable();
        }
        public void coins1k()
        {
           // FindObjectOfType<Purchaser>().BuyConsumablecredit1000();
        }
        public void coins2k()
        {
           // FindObjectOfType<Purchaser>().BuyConsumablecredit2000();
        }
        public void coins5k()
        {
          //  FindObjectOfType<Purchaser>().BuyConsumablecredit5000();
        }
    }
