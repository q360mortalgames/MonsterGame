...
namespace RGSK
{
	public class RaceManager : MonoBehaviour
	{
		...
		//The total number of racers (player included)
		public int playerStartRank = 4;
		//The rank you will start the race as
		...
		//Your race track's distance.
		public float countdownDelay = 3.0f;
		private float countdownTimer = 1.0f;
		public float initialCheckpointTime = 10.0f;
		//start time (Checkpoint race);
		public float driftTimeLimit = 60;
		//time limit (Drift race)
		public float eliminationTime = 30f;
		//start time (Elimination race);
		public float eliminationCounter;
		//timer for elimination
		...
		public bool continueAfterFinish = true;
		//Should the racers keep driving after finish.
		public bool showRacerNames = true;
		//Should names appear above player cars
		public bool showRacerPointers = true;
		//Should minimap pointers appear above all racers
		public bool showRaceInfoMessages = true;
//Show final lap indication , new best lap, speed trap & racer knockout information texts
		public bool forceWrongwayRespawn;
		//should the player get respawned if going the wrong way
		public bool raceStarted;
		//has the race began
		public bool raceCompleted;
		//has the race began
		public bool loadRacePreferences;
		//Load menu prefrences?
		public bool allowDuplicateRacers;
//allow duplicate AI
		...
		public bool autoStartReplay;
		//automatically start the replay after finishing the race
		...
		public void PauseRace ()
		{
			...
			if (raceCompleted || _raceState == RaceState.StartingGrid)
				return;
		...
		{
			if (!raceStarted || _raceState == RaceState.Complete)
				return;
			...
			if (eliminationCounter <= 0) {
				...
				if (RankManager.instance.currentRacers > 1) {
					KnockoutRacer (GetLastPlace ());
				}
		...
		{
			if (ReplayManager.instance.TotalFrames <= 0)
				return;
